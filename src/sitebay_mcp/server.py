...
async def sitebay_list_sites(ctx: Context, team_id: Optional[str] = None) -> str:
    ...
    """
    try:
        ctx.logger.info("Fetching WordPress sites from SiteBay")
        if team_id:
            ctx.logger.info(f"Filtering by team ID: {team_id}")
        
        client = await initialize_client()
        result = await sites.sitebay_list_sites(client, team_id)
        
        ctx.logger.info("Successfully retrieved site list")
        return result
        ...
        ctx.logger.error(f"Unexpected error listing sites: {str(e)}")
